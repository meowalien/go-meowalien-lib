...
func newWithLineErrorFromAny(err any, caller string, obj ...any) *withLineError {
	if err == nil {
		if len(obj) == 0 {
			return nil
		}
		return newWithLineErrorFromAny(obj[0], caller, obj[1:]...)
	}
...
func (w *withLineError) Wrap(a any, caller string) (res *withLineError) {
	ne := newWithLineErrorFromAny(a, caller)
	ne.layer = w.layer + 1
	ne.parent = w
	return ne
}
